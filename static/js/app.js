document.addEventListener("DOMContentLoaded", function() {
  ...
  var buttonToChangeClass = document.querySelector("#set-hidden-class");
  var categoriesList = document.querySelectorAll("input[name='categories']");
  var checkedValues = [];

  buttonToChangeClass.addEventListener("click", function(){
    for (i = 0; i < categoriesList.length; i++){
      if(categoriesList[i].checked) {
        checkedValues.push(categoriesList[i].value);
      }
    }
    for (j = 0; j < institutions.length; j++){
      for (k = 0; k < checkedValues.length; k++){
        if (! institutions[j].classList.contains(checkedValues[k])){
          institutions[j].style.display = 'none'
        }
      }
    }
  });
  ...
  var postcodeOutput = document.querySelector("#postcode");
  var phoneInput = document.querySelector("input[name='phone']");
  var phoneOutput = document.querySelector("#phone");
  ...
  var infoInput = document.querySelector("textarea[name='more_info']");
  var infoOutput = document.querySelector("#more_info");
